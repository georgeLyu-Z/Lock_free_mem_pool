...
// 简化的原子操作宏
#define ATOMIC_BATCH_START() ARM_DMB()
#define ATOMIC_BATCH_END() ARM_DMB()
...
// 编译优化宏
    ...
    #define PREFETCH(addr, rw, locality) __builtin_prefetch(addr, rw, locality)
        ...
        #define ARM_DMB() __asm__ __volatile__("dmb ish" ::: "memory")
        #define ARM_DSB() __asm__ __volatile__("dsb ish" ::: "memory")
    #else
        #define ARM_YIELD() __asm__ __volatile__("yield" ::: "memory")
...
// ARM架构缓存行大小定义
    ...
    #define CACHE_LINE_SIZE 32  // ARMv7: 32字节缓存行
#else
    #define CACHE_LINE_SIZE 64  // ARM64和通用ARM: 64字节
#endif
...
#define ATOMIC_ALIGN __attribute__((aligned(16)))

// 对齐检查宏
#define IS_ALIGNED(ptr, align) (((uintptr_t)(ptr) & ((align) - 1)) == 0)
#define ENSURE_ALIGNMENT(ptr, align) \
    do { if (UNLIKELY(!IS_ALIGNED(ptr, align))) { /* 对齐错误 */ } } while(0)
...
typedef struct header {
    atomic_size_type_t size STRICT_ALIGN;
    atomic_ptr_t next_free STRICT_ALIGN;
    atomic_uint32_t magic ATOMIC_ALIGN;
    atomic_uint32_t version;
    atomic_char is_free;
    char reserved[HEADER_PADDING];
...
typedef struct footer {
    ...
    char reserved[FOOTER_PADDING];
} ATOMIC_ALIGN footer_t;
...
_Static_assert(sizeof(footer_t) == 16, "Footer size must be 16 bytes");
_Static_assert(CACHE_LINE_SIZE == 32 || CACHE_LINE_SIZE == 64, "Cache line must be 32 or 64 bytes");
_Static_assert(ALIGNMENT >= 8, "Requires at least 8-byte alignment");
#if UINTPTR_MAX == 0xFFFFFFFF
...
typedef struct page_header CACHE_ALIGNED {
    ...
    atomic_size_type_t block_count;
...
static inline int cas_next_free(header_t *h, header_t *expected, header_t *desired) {
    uintptr_t exp = CAST_FROM_PTR(expected);
    return atomic_compare_exchange_weak_explicit(&h->next_free, &exp, 
                                               CAST_FROM_PTR(desired),
                                               memory_order_acq_rel, memory_order_acquire);
}

// 统计更新宏（优化）
#define UPDATE_STATS(p, alloc_change, free_change, frag_change) do { \
    if (LIKELY((alloc_change) != 0)) atomic_fetch_add_explicit(&(p)->total_allocated, (atomic_size_type_t)(alloc_change), memory_order_relaxed); \
    if (LIKELY((free_change) != 0)) atomic_fetch_add_explicit(&(p)->total_free, (atomic_size_type_t)(free_change), memory_order_relaxed); \
...
// 兼容性检查
static int compatibility_check() {
    return (CACHE_LINE_SIZE == 32 || CACHE_LINE_SIZE == 64) && 
           ALIGNMENT >= 8 && sizeof(header_t) == CACHE_LINE_SIZE ? 0 : -1;
}
...
static inline int is_block_valid(header_t *b) {
    ...
    // 增强安全检查
    if (magic != MAGIC || size < MIN_BLOCK_SIZE || (size & (ALIGNMENT - 1))) return 0;
    if (size > PAGE_SIZE) return 0; // 防止异常大块
    ...
    // 检查footer是否在合理范围内
    if ((char*)f < (char*)b + sizeof(header_t) || 
        (char*)f + sizeof(footer_t) > (char*)b + size) return 0;
...
static inline int is_block_valid_fast(header_t *b) {
    ...
    uint32_t magic = FAST_LOAD_MAGIC(b);
    if (UNLIKELY(magic != MAGIC)) return 0;
    size_t size = FAST_LOAD_SIZE(b);
    return LIKELY(size >= MIN_BLOCK_SIZE && size <= PAGE_SIZE);
}
...
int mempool_init(size_t hash_size) {
    ...
    if (pool->hash_table == MAP_FAILED) {
        if (munmap(pool, sizeof(mem_pool_t)) != 0) {
            // pool munmap失败，但继续清理
        }
        pool = NULL;
        atomic_store_explicit(&init_flag, 0, memory_order_release);
    ...
    if (!allocate_page()) {
        atomic_store_explicit(&pool->is_initialized, 0, memory_order_release);
        if (munmap(pool->hash_table, sizeof(atomic_uintptr_t) * hsize) != 0) {
            // hash_table munmap失败，但继续清理
        }
        if (munmap(pool, sizeof(mem_pool_t)) != 0) {
            // pool munmap失败，但继续清理
        }
...
void mempool_destroy() {
    ...
    // 先释放哈希表
    if (hash_table) {
        if (munmap(hash_table, sizeof(atomic_uintptr_t) * hash_size) != 0) {
            // 哈希表munmap失败，但继续清理
        }
    }
...
void *mempool_alloc(size_t size) {
    ...
    // 安全的大小计算（防止整数溢出）
    size_t total_overhead = sizeof(header_t) + sizeof(footer_t) + ALIGNMENT - 1;
    if (size > SIZE_MAX - total_overhead) {
        return NULL; // 防止整数溢出
    }
...
void mempool_print_stats() {
    ...
    printf("\n=== Architecture Info ===\n");
#if defined(__aarch64__)
    printf("Variant: AArch64\n");
#elif defined(__ARM_ARCH_7A__)
    printf("Variant: ARMv7-A\n");
#elif defined(__ARM_ARCH_7R__)
    printf("Variant: ARMv7-R\n");
#elif defined(__ARM_ARCH_7M__)
    printf("Variant: ARMv7-M\n");
#else
    printf("Variant: Generic ARM\n");
#endif
